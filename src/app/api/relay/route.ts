...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (fetchError || !userData) {
      console.log('User not found, creating new user. Fetch error:', fetchError);
      ...
      if (orgError || !orgs || orgs.length === 0) {
        ...
        return NextResponse.json({ error: 'No organizations available' }, { status: 500 });
      }
    ...
    console.log('Inserting user message:', { id: messageId, content, userId: user.id, organizationId: userData.organizationId });
    // Insert user message
